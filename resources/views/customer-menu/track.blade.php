...
<main class="wrap"><section class="card" id="trackingCard">
    ...
    <div class="sectionTitle"><h2>ملخص الطلب</h2><span class="live">التحديث تلقائيًا كل 5 ثوانٍ</span></div>
...
const label=document.querySelector('#label span:last-child');const card=document.getElementById('trackingCard');const stages=[...document.querySelectorAll('[data-stage]')];const eta=document.getElementById('eta');const paymentBox=document.getElementById('paymentBox');
...
function render(data){const state=String(data.state||'received').toLowerCase();const cancelled=state==='cancelled';const stage=cancelled?0:Math.max(0,Math.min(3,Number(data.step||1)-1));stages.forEach(el=>{const v=Number(el.dataset.stage);el.classList.toggle('on',!cancelled&&v<=stage);el.classList.toggle('current',!cancelled&&v===stage)});card.classList.toggle('cancelled',cancelled);label.textContent=data.label||'جاري تجهيز طلبك';const minutes=Number(data.estimated_ready_minutes);eta.textContent=!cancelled&&Number.isFinite(minutes)&&minutes>0?`⏱️ الوقت المتوقع لتجهيز طلبك: حوالي ${minutes} دقيقة`:'';eta.classList.toggle('show',!!eta.textContent);
...
const invoice=data.invoice;const invoiceAction=document.getElementById('invoiceAction');if(invoice?.url){document.getElementById('invoiceLink').href=invoice.url;invoiceAction.classList.add('show')}else invoiceAction.classList.remove('show');}
async function poll(){try{const r=await fetch(statusUrl,{headers:{Accept:'application/json'},cache:'no-store'});const d=await r.json();if(!r.ok)throw d;const nextFingerprint=d?.fingerprint||null;if(lastFingerprint&&nextFingerprint&&nextFingerprint!==lastFingerprint){window.CustomerOrderNotify?.changed('تحديث على طلبك',d.label||d?.payment?.message||'تم تحديث حالة طلبك');}lastFingerprint=nextFingerprint||lastFingerprint;render(d)}catch(e){label.textContent='تعذر تحديث الحالة مؤقتًا'}}
render(initial);setInterval(poll,5000);
</script>
</body></html>