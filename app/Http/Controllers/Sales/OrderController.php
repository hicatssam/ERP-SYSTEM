...
class OrderController extends Controller
{
    ...
    public function update(Request $request, Order $order)
    {
        ...
        $data = $request->validate([
            ...
            'notes' => ['nullable','string','max:1000'], 'items' => ['nullable','array'],
            'items.*.id' => ['required_with:items','integer',Rule::exists('order_items','id')->where(fn ($q) => $q->where('order_id',$order->id))],
            'items.*.quantity' => ['required_with:items','numeric','min:0.001'], 'items.*.kitchen_notes' => ['nullable','string','max:500'],
        ]);
    ...
    public function confirm(Request $request, Order $order)
    {
        ...
        try {
            ...
            return back()->withErrors(['order_confirmation' => $messages ?: ['تعذر تأكيد الطلب. راجع المخزون وبيانات الطلب ثم حاول مرة أخرى.']])->withInput();
        } catch (\Throwable $e) {
            report($e);
            return back()->withErrors(['order_confirmation' => app()->isLocal() ? $e->getMessage() : 'تعذر تأكيد الطلب بسبب خطأ غير متوقع. لم يتم تغيير حالة الطلب.'])->withInput();
        }
    ...
    {
        $this->authorize('cancel',$order); $this->ensureOrderAccess($order,$request->user());
    ...
    {
        $this->authorize('update',$order); $this->ensureOrderAccess($order,$request->user()); abort_unless($order->isCustomerMenuOrder(),404);
        $data=$request->validate(['customer_status_message'=>['nullable','string','max:500']]);
        $message=trim((string)($data['customer_status_message']??''));
        $order->forceFill(['customer_status_message'=>$message!==''?$message:null,'customer_status_message_updated_at'=>now(),'customer_status_message_by'=>$request->user()->id])->save();
    ...
    private function releaseCustomerMenuTableIfIdle(Order $order, User $user): void
    {
        ...
        $session=$order->restaurantTableSession()->first(); if(!$session||!$session->isOpen()) return;
