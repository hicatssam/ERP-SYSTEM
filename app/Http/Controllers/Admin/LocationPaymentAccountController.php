...
class LocationPaymentAccountController extends Controller
{
    public function index(Location $location): View
    {
        $accounts = LocationPaymentAccount::query()
            ...
            ->with('locationPaymentMethod.paymentMethod')
            ->orderBy('sort_order')
            ->orderBy('id')
            ->get();

        // Only payment methods enabled for this branch may receive accounts.
        $locationPaymentMethods = LocationPaymentMethod::query()
            ...
            ->with('paymentMethod')
            ->get()
            ->sortBy(fn (LocationPaymentMethod $item) => [
                $item->paymentMethod?->sort_order ?? PHP_INT_MAX,
                $item->paymentMethod?->id ?? PHP_INT_MAX,
            ])
            ->values();

        // Keep the current Blade contract until the UI is migrated to submit
        // location_payment_method_id directly.
    ...
    public function store(Request $request, Location $location): RedirectResponse
    {
        ...
        LocationPaymentAccount::query()->create([
            ...
            // Compatibility columns for existing code during the transition.
